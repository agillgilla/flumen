...
function addSongToQueue(songName, songIndex) {
	...
	row.innerHTML = `
		<td class="table-button"><i class="removeFromQueue fa fa-times-circle fa-5x"></i></td>
		<td class="table-button"><i class="moveUp fa fa-arrow-circle-up fa-5x"></i></td>
		<td class="table-button"><i class="moveDown fa fa-arrow-circle-down fa-5x"></i></td>
...
function buildPlaylist() {
	...
	fetchedSongs.forEach((song, index) => {
		...
		<td name="${song.file}" class="table-button"><i class="addToQueue fa fa-plus-circle fa-5x"></i></td>
		<td align="left"><span class="songName">${song.title}</span><br><span class="artistName">${song.artist}</span></td>
		<td align="left" class="song-duration">${secondsToHms(song.duration)}</td>
		</tr>
		`;
